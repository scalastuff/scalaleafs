...
dTree.prototype.open = function (url) {
    for (var n = 0; n < this.aNodes.length; n++) {
        if (this.aNodes[n].url == url) {
            this.openTo(this.aNodes[n].id, true);
            break;
        }
    }
};
...
dTree.prototype.node = function (node, nodeId) {
    ...
    if (this.config.useIcons) {
        ...
        str += '<img style="vertical-align: middle" width="15px" height="15px" id="i' + this.obj + nodeId + '" src="' +
...
dTree.prototype.indent = function (node, nodeId) {
    ...
    if (this.root.id != node.pid) {
        for (var n = 0; n < this.aIndent.length; n++) {
            str += '<img style="vertical-align: middle" width="15px" height="15px" src="' +
                ( (this.aIndent[n] == 1 && this.config.useLines) ? this.icon.line : this.icon.empty ) + '" alt="" />';
        }
        (node._ls) ? this.aIndent.push(0) : this.aIndent.push(1);
        if (node._hc) {
            str += '<a href="javascript: ' + this.obj + '.o(' + nodeId +
                ');"><img style="vertical-align: middle" width="15px" height="15px" id="j' + this.obj + nodeId +
                '" src="';
